...
def pip_short():
    ...
    with tempfile.TemporaryDirectory() as d:
        os.system(
            f". /app/.venv/bin/activate && pip install --no-index -t {d} {SHORT_PIP_SRC_PATH}/*"
        )
...
def pip_long():
    ...
    with tempfile.TemporaryDirectory() as d:
        os.system(
            f". /app/.venv/bin/activate && pip install --no-index -t {d} {LONG_PIP_SRC_PATH}/*"
        )
...
@benchmark(times=5)
def new_commands():
    os.system("git clone https://github.com/boto/botocore.git")
    os.system("pip install -r requirements-dev.txt --target lib")
